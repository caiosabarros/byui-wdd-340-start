...
-- Explore the PostgreSQL Replace function Do NOT retype the 
-- entire description as part of the query.. It needs to be part
-- of an Update query as shown in the code examples of the SQL 
-- Reading - Read Ch. 1, section 3.

UPDATE public.inventory SET inv_description = REPLACE(inv_description, 'small interiors', 'a huge interior') WHERE inv_id = 10;

-- Use an inner join to select the make and model fields from 
-- the inventory table and the classification name field from 
-- the classification table for inventory items that belong to 
-- the "Sport" category. These resources may help you:
...
ON c.classification_id = 2;

-- Update all records in the inventory table to add "/vehicles" to the middle of the file path in the inv_image and inv_thumbnail columns using a single query. This reference may prove helpful - https://www.postgresqltutorial.com/postgresql-string-functions/postgresql-replace/. 
